...
async function enlist(member, divKey, robloxName) {
  ...
  try {
    const enlisted = member.guild.roles.cache.get(config.roles.enlisted);
    const divRole  = member.guild.roles.cache.get(div.id);
    if (enlisted) await member.roles.add(enlisted).catch(e => errors.push(e.message));
...
async function discharge(member) {
  ...
  const enlisted = member.guild.roles.cache.get(config.roles.enlisted);
  if (enlisted && member.roles.cache.has(config.roles.enlisted)) {
    await member.roles.remove(enlisted).catch(e => errors.push(e.message));
  }
  await member.setNickname(null).catch(e => errors.push(e.message));
  return errors;
}

module.exports = { cap, parseDivision, parseActivity, buildNickname, ts, tsF, isStaff, getDivision, enlist, discharge, DIVISIONS };
