...
async function getUser(username) {
  try {
    ...
    const profile = await axios.get(
      'https://users.roblox.com/v1/users/' + users[0].id,
      { timeout: 8000 }
    );
    ...
    try {
      const av = await axios.get(
        'https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=' + users[0].id + '&size=150x150&format=Png',
        { timeout: 8000 }
      );
...
function ageString(iso) {
  ...
  return Math.floor((Date.now() - new Date(iso)) / 86400000) + ' days';
}
